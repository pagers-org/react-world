...
describe('GNB', () => {
  ...
  // 페이지 이동을 어떻게 테스트할까?
  // - 세부 구현사항을 테스트하지마라
  // - 유저 행동과 최대한 유사한 테스트코드를 작성하라
  //
  // 1. url로 테스트 <-- 페이지 이동은 성공했지만 유저가 해당 페이지에서 정상적인 컨텐츠를 볼 수 없을 수 있다. (유저는 url을 확인하지 않는다. <- 유저행동과 거리가 멂)
  // 2. 해당 페이지에 컨텐츠를 테스트 <-- 해당 페이지에 컨텐츠가 바뀔 확률이 있다. (이는 유저는 행동과 가깝다. 그럼 GNB와 하위 컴포넌트를 같이 불러와야하는데? 그럼 그게 GNB테스트는 아니지 않을까? 앱 전체 테스트인것같은데..)
